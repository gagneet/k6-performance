...
- `app/audit_runner.py` — new module, subprocess orchestration for CodeAnalysis + RepoLens
...
- `scripts/run-audit.sh` — thin wrapper so CodeAnalysis lives inside the repo
...
Let me quickly scan `index.html` to understand its JS structure so my patch fits cleanly:Good. I have everything I need. Now I'll build the full patch. Let me set up the output directory.Now I'll write each file. Starting with the core engine — the audit runner that orchestrates CodeAnalysis and RepoLens as subprocesses, following the exact pattern already in `main.py`.Now the findings parser — this takes Ralph's markdown output and turns it into structured rows:Now let me write a small test harness to validate the parser against realistic CodeAnalysis output before I commit to the schema:Parser works correctly on realistic data and is robust against malformed input. I did spot one small thing during testing: the `src/api/*.py` glob-style path falls through to `file=""` because `_PATH_RE` doesn't match paths with wildcards — that's correct behavior (we shouldn't pretend to know a specific file), and the finding still gets stored.
...
- **`app/audit_runner.py`** — async subprocess orchestration for CodeAnalysis + RepoLens. Resolves targets, detects git SHA, streams output, enforces a 2-hour timeout ceiling, clamps user scope to sane ranges.
- **`app/findings_parser.py`** — parses Ralph's markdown into structured `Finding` rows with severity/type/file/line/confidence. Handles RepoLens JSON output too. Tested against realistic CodeAnalysis output (parses 5/5 findings correctly) and malformed/empty input (no crashes).
...
scripts/code-audit.sh         ← NEW (your code-audit.sh bundled for the container)
...
- Parser: 5/5 structured findings extracted from realistic CodeAnalysis output; survives malformed and empty input
...
2. **RepoLens isn't bundled.** It's a big bash tool with its own prerequisites (`gh` CLI + 280 lens files). The portal will show it as "not installed" in the backend dropdown until you install it — either inside a derived Dockerfile or as a mounted volume. CodeAnalysis works out of the box; RepoLens is there for when you want the deeper scan.

3. **Cost guard.** Scope is clamped in `audit_runner.normalize_scope()` — max 500 CodeAnalysis iterations, max $200 RepoLens budget — but nothing stops someone kicking off 20 audits in parallel. If that's a concern, add a queue in front of `execute_audit` in a v2.
